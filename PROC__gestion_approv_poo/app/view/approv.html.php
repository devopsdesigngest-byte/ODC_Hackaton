            ...
            <div>
                ...
                <div class="panel-card" style="margin-bottom: 0;">
                    ...
                    <table class="debt-table" style="font-size: 12px;">
                        ...
                        <tbody>
                            <?php $concordance = $getAllConcordance ?? [];
                            foreach($concordance as $ccd): ?>
                                <tr>
                                    <td style="font-weight: 700;"><?php echo $ccd ['refbl'] ?></td>
                                    <td><?php echo $ccd ['nom'] ?></td>
                                    <td><?php echo $ccd ['valeurfacturee'] ?></td>
                                    <td><?php echo $ccd ['valeurreceptionnee'] ?></td>
                                    <td style="color: 
                                        <?php if($ccd ['diagnostic'] == 'concorde') echo 'var(--success)';
                                            else if($ccd ['diagnostic'] == 'en attente') echo 'var(--warning)';
                                            else echo 'var(--danger)'; ?>; font-weight: 700;">
                                            <?php echo $ccd ['diagnostic'] ?>
                                    </td>
                                </tr>
                            <?php endforeach;?>
                        </tbody>
                        ...
                        <div class="form-group">
                            ...
                            <div style="position: relative;">
                                <select id="supplier-select" class="form-control" style="width: 100%; appearance: none; padding-right: 30px;">
                                    ...
                                    <?php 
                                        $allFournisseur = $getAllFournisseur ?? [];
                                        foreach($allFournisseur AS $four): 
                                     ?>
                        ...
                        <div style="border-top: 1px dashed var(--border-color); padding-top: 16px; margin-top: 16px; margin-bottom: 16px;">
                            ...
                            <div style="display: grid; grid-template-columns: 2fr 1fr auto; gap: 12px; align-items: flex-end; margin-bottom: 16px;">
                                <div class="form-group" style="margin-bottom: 0;">
                                    ...
                                    <select id="pos-item-select" class="form-control" style="background-color: #0b0f1a; color: white; width: 250px;">
                                     <?php 
                                        $allArticles = $getAllArticles ?? [];
                                        foreach($allArticles AS $art): 
                                     ?>    
                                        <option value="<?= $art['prixachatreel'] ?>" data-name="Sac de riz 50kg"><?= $art['libelle'] ?> (Coût d'achat : <?= $art['prixachatreel'] ?> F)</option>
                                    <?php 
                                        endforeach 
                                    ?>
                                    </select>
            ...
            <div>
                ...
                <div class="panel-card" style="margin-bottom: 32px;">
                    ...
                    <div id="slips-container">
                        ...
                        <!-- Slip Card 1 -->
                         <?php  $allApprosFournisseurs = $getFournisseurAppro ?? [];
                   

                        foreach($allApprosFournisseurs as $appro): ?>
                        
                        <?php if($appro['statut'] === 'Receptionnes') { ?>
                        <div class="panel-card" style="padding: 20px; border-radius: 16px; margin-bottom: 16px; background: rgba(255,255,255,0.01);" data-supplier="CCS" data-ref="BL-CCS-098" data-status="receptionne">
                            
                            <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 12px;">
                                <div>
                                    <span style="font-size: 11px; color: var(--text-muted); font-weight: 700;">Réf: <?php echo $appro['refbl'] ?> • <?php echo $appro['dat'] ?></span>
                                    <div style="font-size: 16px; font-weight: 700;"><?php echo $appro['fnom'] ?></div>
                                </div>
                                <span class="badge payee" id="status-1"><?php echo $appro['statut'] ?></span>
                            </div>
                            <div style="display: flex; justify-content: space-between; align-items: center;">
                                <div style="font-size: 18px; font-weight: 800; color: var(--accent);"><?php echo $appro['somme'] ?> FCFA</div>
                                <div style="display: flex; gap: 8px;">
                                    <button class="btn-quick-action" onclick="toggleDetails('details-<?= $appro['id'] ?>')">Voir articles (<?=$appro['cpt']?>)</button>
                                </div>
                            </div>
                            ...
                            <?php foreach($appro['la'] AS $la): ?>
                            <!-- Drawer -->
                                <div style="display: flex; justify-content: space-between; font-size: 12px; color: var(--text-muted);">
                                    <span><?php echo $la['qterecue'] ?>x <?php echo $la['libelle'] ?> (achat <?= $la['prixachatreel'] ?> F/u)"</span>
                                    <span style="font-weight: 700; color: var(--text-main);"> <?php echo $la['montant'] ?> F</span>
                                </div>
                                <?php endforeach ?>
                    ...
                    <?php }else{ ?>
                        ...
                        <!-- Slip Card 2 -->
                        <form method="POST" name="transaction"  class="panel-card" style="padding: 20px; border-radius: 16px; margin-bottom: 16px; background: rgba(255,255,255,0.01);" data-supplier="Grossiste Alimentaire Diop" data-ref="BL-DIP-012" data-status="encours">
                         

                            <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 12px;">
                                <div>
                                    <span style="font-size: 11px; color: var(--text-muted); font-weight: 700;"><?php echo $appro['refbl'] ?> • <?php echo $appro['dat'] ?></span>
                                    <div style="font-size: 16px; font-weight: 700;"><?php echo $appro['fnom'] ?></div>
                                </div>
                                <span class="badge non-payee" id="status-2"><?php echo $appro['statut'] ?></span>
                            </div>
                            <div style="display: flex; justify-content: space-between; align-items: center;">
                                <div style="font-size: 18px; font-weight: 800; color: var(--accent);"><?php echo $appro['somme'] ?> FCFA</div>
                                <div style="display: flex; gap: 8px;">
                                    <button type="button" class="btn-quick-action" onclick="toggleDetails('details-<?=$appro['id']?>')">Voir articles (<?=$appro['cpt']?>)</button>
                                        <!-- Drawer (Interactive Editor) -->
                            ...
                            <div class="details-drawer" id="details-<?=$appro['id']?>">
                                ...
                                <div id="details-2-editor">
                                    ...
                                    <table style="display: flex; flex-direction: column; gap: 10px;">

                                        <?php foreach($appro['la'] AS $la): ?>
                                        <tr style="display: grid; grid-template-columns: 2fr 1fr 1.2fr; gap: 12px; align-items: center; padding-bottom: 8px; border-bottom: 1px solid rgba(255,255,255,0.03);">
                                            
                                            <td>
                                                <span style="font-weight:700; font-size:12px;">
                                                    <?php echo $la['libelle'] ?> 
                                                    (Attendu: <?php echo $la['qteappro'] ?>)
                                                </span>
                                            </td>    
                                            ...
                                            <td>
                                                ...
                                                <input 
                                                    name="prixAchatReel[<?= $la['id'] ?>]"
                                                    type="number" 
                                                    id="recept-price-2-1" 
                                                    class="form-control" 
                                                    value="<?= $la['prixachatreel'] ?>" 
                                                    style="padding:4px 8px; font-size:11px;"
                                                >
                                            </td>
                ...
                <div class="panel-card">
                    ...
                    <div style="display: flex; flex-direction: column; gap: 14px;">
                        ...
                        <?php $articlesR = $articlesRupture;
                        foreach($articlesR as $articleR):?>
                        <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid rgba(255,255,255,0.03); padding-bottom: 8px;">

                            <div>
                                <div style="font-weight: 700; font-size: 13px;"><?php echo $articleR['libelle']  ?></div>
                                <span style="font-size: 11px; color:
                                    <?php 
                                    echo ($articleR['qtestock'] == 0) ? 'var(--danger)' : 'var(--warning)';
                                    ?>;
                                    font-weight: 700;">
                                    <?php echo $articleR['statut']  ?>
                                </span>
                            </div>
                            <button class="btn-quick-action" onclick="toggleOrderDraft('draft-1', 'Bidon d\'huile 5L', 'Grossiste Diop & Frères', 50)">Commander</button>
                        </div>
                        <?php endforeach; ?>                        <div class="order-draft-panel" id="draft-1">
                            <div style="font-weight: 700; margin-bottom: 6px; color: var(--accent);">Demande d'Approvisionnement Automatique</div>
                            <textarea class="draft-textarea" id="text-draft-1"></textarea>
                            <button type="button" class="btn-quick-action" style="font-size: 10px; width: 100%; border-color: var(--success); color: var(--success);" onclick="copyDraft('text-draft-1')">Copier le bon de commande</button>
                        </div>
    ...
    <div class="app-container">
        ...
        <div class="navbar">
            <div class="nav-logo">
                <span>📦</span> SupplyPro Pro
                <span
                    style="font-size: 12px; font-weight: 500; color: var(--text-muted); margin-left: 10px; border-left: 1px solid var(--border-color); padding-left: 10px;">Console
                    d'Approvisionnement</span>
            </div>
            <div class="system-status">
                <div class="status-pill online">Dépôt Principal Active</div>
                <div class="status-pill">Bordereaux Automatisés</div>
                <a href="debt_dashboard.html" class="back-link"
                    style="margin-left: 10px; color: var(--text-muted); text-decoration: none; font-weight: 600; font-size: 13px;">Dashboard
                    →</a>
            </div>
        </div>
        ...
        <div class="main-layout">
            ...
            <div>
                <!-- 700px Inbound POS Form -->
                <div class="panel-card"
                    style="padding: 24px; border: 1px solid rgba(14, 165, 233, 0.2); background: linear-gradient(180deg, rgba(17, 24, 43, 0.5) 0%, rgba(10, 15, 30, 0.3) 100%);">
                    <div class="panel-title"
                        style="border-left-color: var(--accent); display: flex; justify-content: space-between; align-items: center;">
                        <span>🚚 Saisie d'Approvisionnement</span>
                        <span
                            style="font-size: 11px; font-weight: 600; color: var(--text-muted); background: rgba(255,255,255,0.03); padding: 4px 8px; border-radius: 6px;">Nouveau
                            Lot</span>
                    </div>
                    <form id="supply-mock-form" action="http://localhost:8000/save" method="POST">

                        <div class="form-group">
                            <label for="supplier-select">Fournisseur Partenaire</label>
                            <div style="position: relative;">
                                <select id="supplier-select" name="fournisseur" class="form-control"
                        ...
                        <div
                            ...
                            <label
                                ...
                                des Articles & Coûts d'Achat</label>
                            <div
                                style="display: grid; grid-template-columns: 2fr 1fr auto; gap: 12px; align-items: flex-end; margin-bottom: 16px;">
                                <div class="form-group" style="margin-bottom: 0;">
                                    <label for="pos-item-select">Article</label>
                                    <select id="pos-item-select" name="article" class="form-control"
                                        style="background-color: #0b0f1a; color: white;">
                                        <option value="0"></option>
                                        <?php
                                        $articles = $articles ?? [];
                                        foreach ($articles as $article):
                                            ?>
                                            <option value="<?= $article['id']."_{$article['libelle']}_{$article['prixachat']}" ?>">
                                                <?= $article['libelle'] . "(Coût d'achat : {$article['prixachat']} F)" ?>
                                            </option>
                                        <?php endforeach ?>
                                    </select>
                                </div>
                                <div class="form-group" style="margin-bottom: 0;">
                                    <label for="pos-qty">Quantité Lot</label>
                                    <input type="number" name="quantite" id="pos-qty" class="form-control" value="0" min="0"
                                        style="padding: 12px 10px;">
                                </div>
                                <button type="submit" name="btnActionAppro" value="saveSession" class="btn-submit"
                                    style="height: 46px; width: 46px; font-size: 18px; display: flex; justify-content: center; align-items: center; background: linear-gradient(135deg, var(--accent) 0%, #0369a1 100%); font-weight: bold; border-radius: 10px;">+</button>
                            </div>
                            ...
                            <table class="debt-table" style="font-size: 12px;">
                                ...
                                <tbody id="cart-rows">
                                        ...
                                        <?php
                                        $lignes = $_SESSION['appro']['lignes'] ?? [];
                                        $total = $_SESSION['appro']['montant'] ?? 0;
                                        if (empty($lignes)):
                                            ...
                                            <td colspan="4"
                                                ...
                                                Aucun article dans ce lot. Ajoutez des lignes.</td>
                                            
                                            </tr>
                                        <?php else: ?>
                                            <?php foreach ($lignes as $key => $ligne): ?>
                                            <tr>
                                                <td style="padding: 8px 0; font-weight:700;"><?= $ligne['libelle'] ?></td>
                                                <td style="padding: 8px 0;"><?= $ligne['quantite'] ?></td>
                                                <td style="padding: 8px 0; font-weight:800; color:var(--accent);"><?= $ligne['montant'] ?> F</td>
                                                <td style="padding: 8px 0; text-align:right;">
                                                    <button type="submit" name="btnActionAppro" value="removeSession"
                                                        style="background:none; border:none; color:var(--danger); cursor:pointer; font-size:14px;">🗑️</button>
                                                </td>
                                            </tr>
                                        <?php endforeach ?>
                                    <?php endif ?>
                        ...
                        <!-- Ecran de Facture Digital -->
                        <div
                            style="background: linear-gradient(135deg, rgba(14, 165, 233, 0.08) 0%, rgba(30, 41, 59, 0.4) 100%); border: 1px solid rgba(14, 165, 233, 0.15); border-radius: 16px; padding: 18px; text-align: center; margin-bottom: 20px; box-shadow: inset 0 0 15px rgba(14, 165, 233, 0.08);">
                            <span
                                style="font-size: 11px; color: var(--text-muted); text-transform: uppercase; font-weight: 700; letter-spacing: 1px; display: block; margin-bottom: 4px;">Valorisation
                                Globale du Lot d'Entrée</span>
                            <div
                                style="font-size: 28px; font-weight: 900; color: #38bdf8; letter-spacing: -0.5px; font-family: monospace; text-shadow: 0 0 10px rgba(56, 189, 248, 0.3);">
                                <span id="montant_total_display_text"><?= $total ?></span> <span
                                    style="font-size: 16px; font-weight: 700;">FCFA</span>
                            </div>
                        </div>

                        <div class="form-group" style="margin-bottom: 24px;">
                            <label for="reference-bordereau">Référence du Bordereau de Livraison (Fournisseur)</label>
                            <input type="text" name="bl" id="reference-bordereau" class="form-control"
                                placeholder="Ex: BL-CCS-2026-98">
                        </div>

                        <button type="submit" name="btnActionAppro" value="saveAppro" class="btn-submit btn-success"
                            style="padding: 16px 24px; font-weight: 800; font-size: 14px;">Enregistrer & Augmenter
                            Stocks (DML)</button>
                    </form>
                </div>
