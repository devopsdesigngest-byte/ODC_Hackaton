...
class POSController {
    ...
    public static function showVue(): void {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            if ($_POST['btn'] === 'panier') 
                self::ajouterDansPanier();      
            if ($_POST['btn'] === 'valider') 
                self::enregistrerVente();
        }

        $clients = ClientRepository::getAllClient();
        $produits = ProduitRepository::getAllProduit();
        $modes = ModePaiementRepository::getAllModePaiement();
    ...
    public static function ajouterDansPanier(): void {
    ...
    public static function enregistrerVente(): void {
...
//             $_SESSION['panier'] = [];
//         }
//         $_SESSION['panier'][] = ['produitId' => (int) $produit[0], 'prixUnitaire' => (float) $produit[1], 'stock' => (int) $produit[2], 'libelle' => $produit[3], 'quantite' => $quantite, 'sousTotal' => (float) $produit[1] * $quantite];
//     }


//     public function enregistrerVente() : void {
//         $venteService = new VenteService();
//         $venteService->enregistrerVente((int) $_POST['client_id'], (int) $_SESSION['utilisateur_id'], (int) $_POST['mode_reglement'], (float) $_POST['montant_verse'], $_SESSION['panier']);
//         unset($_SESSION['panier']);
//     }
// }